...
var documents = [{
    ...
    "body": "2023/12/05 - ROS2, MoveIt, RViz Overview: Goal: Develop a ROS2 package that uses a 7 DoF Framka Emika Panda robotic arm to write translated text and speech on a whiteboard. GitHub: https://github. com/ME495-EmbeddedSystems/final-project-dkoh555 Teammates: Allen Liu, Kassidy Shedd, Megan Black, Damien Koh Process:  Calibrate     The robot uses an Intel Realsense camera to acquire the location of three AprilTags, which, together, are used to constrain the plane of the whiteboard.    The distance and orientation of the whiteboard is made public through a custom message type on a ROS2 topic.    Using the known transform between the camera link and the robot arm’s base link, the updated configuration (Pose) of the whiteboard is extrapolated into the frame of reference of the end-effector and is used to position the pen when writing.     Detect Text     By default, the robot then enters the “Detecting” state, in which it waits for a human to enter the frame and start writing on the whiteboard.    Using the YOLOv8 deep learning model, the robot recognizes once a human has entered the frame and then left.    Next, the robot uses the PaddleOCR library to detect the text written on the whiteboard and pass it along, without any processing, as a string. The desired language is written as a short language code (e. g. “en” for English) above the unknown word(s).    Alternatively, instead of writing a word on the whiteboard, the user could call the speech service which activates the microphone as an input. The user then simply speaks out loud, and the word(s) are passed along as a string from there. By default, the desired language for speech is set to English.     Translate Text     The next stage of the pipeline is the translation node which takes in both the desired language and the string of the unknown word(s).    The translation node uses the Google Translate API and can translate to and from 50+ languages.    The output of this step is a fully-translated string of text.     Text to Waypoints     The string2waypoints node uses matplotlib to convert each character to a series of waypoints–passed along as Point messages–which can then be followed by the robot arm.     Waypoints to Movement        "
    }, {
    "id": 6,
